    ...
    pub fn restore(&mut self, paths: Vec<BackupPath>) {
        ...
        std::thread::spawn(move || {
            ...
            for path in paths {
                ...
                std::fs::create_dir_all(&path.absolute_path).unwrap();
                send(&tx, format!("#Copying to \"{}\" ...", path.absolute_path.display()));
                for content_path in std::fs::read_dir("./content/").unwrap() {
                    fs_extra::copy_items(&[content_path.unwrap().path()], &path.absolute_path, &CopyOptions {
                        overwrite: true,
                        ..Default::default()
                    }).unwrap();
                }
...
fn copy_folder_files_to_folder(from: impl AsRef<Path>, to: impl AsRef<Path>) {
    ...
    for path in std::fs::read_dir(&from).unwrap() {
        let path = path.unwrap().path();
        fs_extra::copy_items(&vec![path], to.as_ref(), &CopyOptions {
            overwrite: true,
            ..Default::default()
        }).unwrap();
    }
